...
mod test {
	...
	fn basic_proof_of_existence() {
		...
		assert_eq!(pallet.claims.len(), 0);
		pallet.create_claim("alice", "hello world").unwrap();
		assert_eq!(pallet.claims.len(), 1);
		assert_eq!(*pallet.get_claim(&"hello world").unwrap(), "alice");
		assert_eq!(
			pallet.create_claim("bob", "hello world"),
			Err("this content is already claimed")
		);
		pallet.revoke_claim("alice", "hello world").unwrap();
		assert_eq!(pallet.claims.len(), 0);
		assert_eq!(pallet.revoke_claim("alice", "hello world"), Err("claim does not exist"));
		pallet.create_claim("alice", "hello world").unwrap();
		assert_eq!(
			pallet.revoke_claim("bob", "hello world"),
